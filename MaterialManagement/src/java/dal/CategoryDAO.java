...
public class CategoryDAO extends DBContext {
    ...
    List<Category> list = new ArrayList<>();
    String sql = "SELECT * FROM Categories WHERE category_id = ? AND category_name LIKE ? AND status = ?";
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
        ps.setInt(1, categoryId);
    ...
    // Tìm kiếm theo ID và Name
public List<Category> searchCategoriesByIdAndName(int categoryId, String keyword) {
    List<Category> list = new ArrayList<>();
    String sql = "SELECT * FROM Categories WHERE category_id = ? AND category_name LIKE ?";
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
        ps.setInt(1, categoryId);
        ps.setString(2, "%" + keyword + "%");
        ResultSet rs = ps.executeQuery();
...
// Tìm kiếm theo ID và Status
public List<Category> searchCategoriesByIdAndStatus(int categoryId, String status) {
    List<Category> list = new ArrayList<>();
    String sql = "SELECT * FROM Categories WHERE category_id = ? AND status = ?";
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
        ps.setInt(1, categoryId);
...
public List<Category> searchCategoriesByNameAndStatus(String keyword, String status) {
    List<Category> list = new ArrayList<>();
    String sql = "SELECT * FROM Categories WHERE category_name LIKE ? AND status = ?";
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
        ps.setString(1, "%" + keyword + "%");
        ps.setString(2, status);
        ResultSet rs = ps.executeQuery();
    ...
    public boolean updateCategory(Category category) {
        String sql = "UPDATE Categories SET category_name = ?, parent_id = ?, created_at = ?, disable = ?, status = ?, description = ?, priority = ? "
                + "WHERE category_id = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, category.getCategory_name());
