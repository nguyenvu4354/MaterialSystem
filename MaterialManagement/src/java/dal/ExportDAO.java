...
public class ExportDAO extends DBContext {
    ...
    public ExportRequest getExportRequestByCode(String requestCode) throws SQLException {
        String sql = "SELECT er.*, u1.full_name AS userName, u2.full_name AS recipientName, u3.full_name AS approverName " +
                     "FROM Export_Requests er " +
                     "LEFT JOIN Users u1 ON er.user_id = u1.user_id " +
                     "LEFT JOIN Users u2 ON er.recipient_user_id = u2.user_id " +
                     "LEFT JOIN Users u3 ON er.approved_by = u3.user_id " +
                     "WHERE er.request_code = ? AND er.disable = 0 AND er.is_used = 0";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
    ...
    public void markExportRequestAsUsed(int exportRequestId) throws SQLException {
        String sql = "UPDATE Export_Requests SET is_used = 1 WHERE export_request_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, exportRequestId);
            stmt.executeUpdate();
        }
    }

    public List<ExportRequestDetail> getExportRequestDetails(int exportRequestId) throws SQLException {
        List<ExportRequestDetail> details = new ArrayList<>();
        String sql = "SELECT erd.*, m.material_name, m.material_code, m.materials_url, u.unit_name " +
        ...
        return details;
    }public RepairRequest getRepairRequestByCode(String requestCode) throws SQLException {
        String sql = "SELECT rr.*, u1.full_name AS userName, u2.full_name AS approverName " +
                     "FROM Repair_Requests rr " +
                     "LEFT JOIN Users u1 ON rr.user_id = u1.user_id " +
                     "LEFT JOIN Users u2 ON rr.approved_by = u2.user_id " +
                     "WHERE rr.request_code = ? AND rr.disable = 0 AND rr.is_used = 0";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, requestCode);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    request.setRequestCode(rs.getString("request_code"));
                    request.setUserId(rs.getInt("user_id"));
                    request.setFullName(rs.getString("userName"));
                    request.setRequestDate(rs.getTimestamp("request_date"));
                    request.setStatus(rs.getString("status"));
                    request.setReason(rs.getString("reason"));
                    request.setApprovedBy(rs.getInt("approved_by"));
    ...
    public void markRepairRequestAsUsed(int repairRequestId) throws SQLException {
        String sql = "UPDATE Repair_Requests SET is_used = 1 WHERE repair_request_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, repairRequestId);
            stmt.executeUpdate();
        }
    }
    ...
    public List<RepairRequestDetail> getRepairRequestDetails(int repairRequestId) throws SQLException {
        ...
        String sql = "SELECT rrd.*, m.material_name, m.material_code, u.unit_name, s.supplier_name " +
